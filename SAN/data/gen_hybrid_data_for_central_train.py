...
def Gen_hybrid(labels_path, train_or_tests):
    class Tree:
    ...
    out = 'data/SAN/' + "SAN_"+ train_or_tests + '_hyb'
    if not os.path.exists(out):
        os.makedirs(out)
    else:
        files = glob.glob(os.path.abspath(os.path.join(out,"*")))
        for f in files:
            os.remove(f)
    ...
    return str(os.path.abspath(out))
